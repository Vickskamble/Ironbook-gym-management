...
-- 1g. STAFF (trainers, receptionists, etc.)
create table if not exists public.staff (
  ...
  phone           text not null,
  email           text default '',
  role            text not null default 'Trainer' check (role in ('Trainer','Receptionist','Cleaner','Manager','Other')),
  salary          numeric(10,2) not null default 0,
  join_date       date not null default current_date,
  status          text not null default 'Active' check (status in ('Active','Inactive','Terminated')),
  profile_pic     text,
  specialization  text default '',
  shift           text default 'Full Day' check (shift in ('Morning','Evening','Full Day')),
  created_at      timestamptz not null default now(),
  updated_at      timestamptz not null default now()
);
...
create index idx_gyms_subscription on public.gyms(subscription);
...
create index idx_members_membership_end on public.members(membership_end);
...
create index idx_staff_gym_id on public.staff(gym_id);
...
create policy "Superadmins can insert gyms"
...
-- 3g. STAFF RLS
create policy "Users can view staff for their gym"
  on public.staff for select
  using (gym_id = public.current_user_gym_id() or is_superadmin());

create policy "Users can insert staff for their gym"
  on public.staff for insert
  with check (gym_id = public.current_user_gym_id() or is_superadmin());

create policy "Users can update staff for their gym"
  on public.staff for update
  using (gym_id = public.current_user_gym_id() or is_superadmin());

create policy "Users can delete staff for their gym"
